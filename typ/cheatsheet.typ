...
Полиномиальный хэш строки: $h = s_0 dot p^0 + s_1 dot p^1 + ... + s_(n-1) dot p^(n-1) space (mod M)$. Рабин—Карп сдвигает окно за $O(1)$ с помощью *rolling hash*: вычитает первый символ, делит на $p$. При совпадении хэшей проверяет строки посимвольно.
#code-with-asymptotics([
```cpp
...
= Префикс-функция
$pi_i$ — длина наибольшего собственного префикса $s[0..i]$, который одновременно является суффиксом. Используется в КМП и для поиска всех вхождений паттерна.
#code-with-asymptotics([
```cpp
...
= Z-функция
$z[i]$ — длина наибольшего префикса строки $s$, совпадающего с подстрокой, начинающейся в позиции $i$. Поддерживается Z-box $[l, r)$: если $i < r$, то $z[i] >= min(r-i, z[i-l])$.
#code-with-asymptotics([
```cpp
...
Склейка `p + '#' + t` позволяет найти все вхождения паттерна $p$ в текст $t$ за один проход: если $pi_i = |p|$, то найдено вхождение в позиции $i - 2|p|$.
#code-with-asymptotics([
```cpp
...
Конечный автомат строится на основе префикс-функции: таблица переходов $delta[q][c]$ — из состояния $q$ по символу $c$ — вычисляется за $O(m dot |Sigma|)$. После построения поиск в тексте длиной $n$ занимает $O(n)$, состояние 0 — начальное, состояние $m$ — паттерн найден.
#code-with-asymptotics([
```cpp
...
```
], [Построение: $O(m dot |Sigma|)$. Поиск: $O(n)$.])
...
```
], [Время: $O(sum |p_i| dot |Sigma| + n + z)$.])

= Хэширование: идея, хэш-таблица, коллизии
Хэш-функция $h: U -> {0,...,m-1}$ отображает ключи в индексы таблицы. Идеальная функция — инъекция, реальная — допускает коллизии. Простой пример: $h(k) = k mod m$, $m$ — простое. *Коллизия* — $h(k_1)=h(k_2)$ при $k_1 != k_2$.
...
= Метод Кукушки
Используются две хэш-таблицы $T_1$ и $T_2$ с функциями $h_1$, $h_2$. Вставка: поместить в $T_1[h_1(k)]$; если занято — вытолкнуть старый ключ в $T_2[h_2(·)]$, и так далее. При цикле (> $N$ перемещений) — перехэширование. Гарантированный $O(1)$ поиск.
#code-with-asymptotics([
```cpp
void insert(int key){
...
= Фильтр Блума
Вероятностная структура данных для проверки принадлежности к множеству. Битовый массив размером $m$, $k$ хэш-функций. *Ложноположительный* ответ возможен, *ложноотрицательный* — нет. Вероятность ложного срабатывания: $approx (1 - e^(-k n \/ m))^k$.
#code-with-asymptotics([
```cpp
bitset<M> bloom;
